...
export default function Globe({
  ...
  useEffect(() => {
    ...
    snapshot.nodes.forEach((n) => {
      ...
      const color = new THREE.Color(n.ground ? '#ffffff' : '#78bac9');
    ...
    if (links) {
      ...
      snapshot.edges.forEach((e, i) => {
        if (e.rate < filterThreshold || e.rate <= 0) return;
    ...
    const visible = snapshot.routes.filter(
      (r) => !tracePath && (overlay || r.name === selected),
    );
    for (const route of visible) {
      ...
      if (route.bottleneck !== null && route.path.length) {
        const edge = snapshot.edges[route.bottleneck];
        const marker = new THREE.Mesh(
          new THREE.OctahedronGeometry(0.016),
          new THREE.MeshBasicMaterial({ color: '#ffcc86' }),
        );
        marker.position.copy(
          v(snapshot.nodes[edge.a].p)
            .add(v(snapshot.nodes[edge.b].p))
            .multiplyScalar(0.5),
        );
        marker.userData.edge = route.bottleneck;
        group.add(marker);
      }
    }
    if (tracePath)
      for (let j = 1; j < tracePath.length; j++) {
        const a = tracePath[j - 1],
          b = tracePath[j];
        const index = snapshot.edges.findIndex(
          (e) => (e.a === a && e.b === b) || (e.a === b && e.b === a),
        );
        draw(a, b, '#ffffff', 1, index);
      }
    snapshot.nodes.forEach((n) => {
      if (n.ground) {
        const marker = new THREE.Mesh(
  ...
  }, [snapshot, selected, overlay, links, trails, filterThreshold, tracePath]);
  return (
    <div
